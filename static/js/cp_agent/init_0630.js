// static/js/cp_agent/init.js - 애플리케이션 초기화 (수정됨)
...
async function initializeApplication() {
    ...
    try {
        ...
        setTimeout(() => {
            const currentTab = $('.preview-tab-btn.border-gray-600').data('tab');
            if (currentTab === 'html') {
                console.log('페이지 로드 시 HTML탭이 활성화되어 있음, 아코디언 표시');
                forceShowHtmlTabAccordions();
            }
        }, 1000);
...
function showKeyboardShortcutsIfFirstVisit() {
    try {
        ...
        if (!hasVisited) {
            setTimeout(() => {
                showToast('💡 Ctrl+S: 저장, Ctrl+Enter: AI 생성, Ctrl+Shift+E: 디버그 패널', 'success');
