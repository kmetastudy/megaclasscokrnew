// static/js/cp_agent/content.js - 향상된 콘텐츠 관리 함수들 (수정됨 - 자동복구 제거)
...
function requestAI() {
    ...
    const formData = {
        ...
        answer_input: window.answerInputEditor ? window.answerInputEditor.getValue() : '{}',
        meta_data: window.metaEditor ? window.metaEditor.getValue() : '{}',
        tags: window.tagsEditor ? window.tagsEditor.getValue() : '{}'
    };
    ...
    // 로딩 스피너 표시
    const spinnerShown = showLoadingSpinner();
    if (!spinnerShown) {
        console.warn('스피너 표시 실패, 계속 진행');
    }
    
    const timer = new window.PerformanceTimer('AI 문항 생성');
...
function reinitializeHtmlTabEditors() {
    ...
    try {
        // CPAgent.Editor의 forceReinitializeHtmlTab 함수 사용
...
function setAnswerToContainer(answerData) {
    try {
        ...
        if (window.answerEditorInstance) {
            window.answerEditorInstance.setValue(answerData);
...
function validateGeneratedContent() {
    ...
    const html = window.htmlEditorInstance ? window.htmlEditorInstance.getValue() : '';
    const answer = window.answerEditor ? window.answerEditor.getValue() : '{}';
...
function applyTemplate(templateId) {
    ...
    $.get(`/cp/api/templates/${templateId}/`)
        .done(function(template) {
            // HTML 구조 적용
            if (template.page && window.htmlEditorInstance) {
                window.htmlEditorInstance.setValue(template.page);
            }
            
            // 답안 구조 적용 (유니코드 디코딩)
            if (template.answer) {
                const decodedAnswer = decodeUnicodeString(template.answer);
                if (window.answerEditor) window.answerEditor.setValue(decodedAnswer);
                if (window.answerInputEditor) window.answerInputEditor.setValue(decodedAnswer);
            ...
            if (template.meta_data) {
                ...
                if (window.metaEditor) window.metaEditor.setValue(decodedMetaData);
            ...
            if (template.tags) {
                ...
                if (window.tagsEditor) window.tagsEditor.setValue(decodedTags);
...
function loadContent(contentId) {
    ...
    const timer = new window.PerformanceTimer('문항 로드');
    ...
    $.get(`/cp/api/contents/${contentId}/`)
        .done(function(data) {
            ...
            // HTML 콘텐츠 설정
            if (data.page && window.htmlEditorInstance) {
                window.htmlEditorInstance.setValue(data.page);
                updatePreview();
            }
...
function saveContent() {
    ...
    if (window.metaEditorInstance_html && window.metaEditor) {
        try {
            window.metaEditor.setValue(window.metaEditorInstance_html.getValue());
        } catch (e) {
            console.warn('메타데이터 동기화 실패:', e);
        }
    }
    if (window.tagsEditorInstance_html && window.tagsEditor) {
        try {
            window.tagsEditor.setValue(window.tagsEditorInstance_html.getValue());
        } catch (e) {
    ...
    const formData = {
        title: $('#title').val(),
        content_type: $('#content_type').val(),
        page: window.htmlEditorInstance ? window.htmlEditorInstance.getValue() : '',
    ...
    const timer = new window.PerformanceTimer('콘텐츠 저장');
    
    const url = window.currentContent ? 
        `/cp/api/contents/${window.currentContent.id}/update/` : 
        '/cp/api/contents/create/';
    const method = window.currentContent ? 'PUT' : 'POST';
...
function createNewContent() {
    ...
    // 임시 파일 정리
    if (window.temporaryUploads && window.temporaryUploads.length > 0) {
