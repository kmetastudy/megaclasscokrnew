// static/js/cp_agent/ui.js - UI 컨트롤 함수들 (수정됨 - 아코디언 단순화)
...
/**
 * HTML탭 에디터들 리프레시 (단순화됨)
 */
function refreshHtmlTabEditors() {
    debugLog('UI', 'HTML탭 에디터 리프레시');
    ...
    editors.forEach(({ name, editor }) => {
        if (editor && typeof editor.refresh === 'function') {
            try {
                editor.refresh();
                debugLog('UI', `${name} 리프레시 완료`);
            } catch (e) {
                console.warn(`${name} 리프레시 실패:`, e);
            }
        }
    });
}
...
function scrollIntoView($element, options = {}) {
    ...
    $element[0].scrollIntoView(finalOptions);
}
...
function showFieldValidation($field, isValid, message = '') {
    ...
    if (isValid) {
        ...
        if (message) {
            $field.after(`<div class="validation-message text-xs text-red-600 mt-1">${message}</div>`);
        }
    }
}
...
function showTooltip($element, message, position = 'top') {
    ...
    const $tooltip = $(`
        <div class="custom-tooltip absolute z-50 px-2 py-1 text-xs text-white bg-gray-800 rounded shadow-lg">
            ${message}
        </div>
    `);
